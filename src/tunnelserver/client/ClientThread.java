...
public class ClientThread extends Thread {
	 ...
	 */
	protected static final String HOST = "192.168.1.44";
	/**
	 * Port
	 */
	...
	public void run() {
		if (count > 0) {
			// TO-DO: kein Guide verfügbar -> was dann?
			guidesMonitor.request();

			// Socketverbindung wird hergestellt
			try {
				...
				clientForm.addStatus("Visit with " + count + " Visitors requested...");
				sout.writeInt(count);
				// wartet auf eine Antwort des Servers
				int serveranswer = sin.readInt();
				if (serveranswer == 1) {
					clientForm.addStatus("Visit with " + count + " Visitors enter the tunnel");
					clientForm.addVisit(count);
				}

				// bei Fehlern wird Guide wieder freigegeben
			...
			try {
				...
				DataOutputStream sout = new DataOutputStream(s.getOutputStream());
				DataInputStream sin = new DataInputStream(s.getInputStream());
				// Besucheranzahl wird negatic geschickt
				sout.writeInt(count);

				int serveranswer = sin.readInt();

				if (serveranswer == 1) {
					clientForm.addStatus("Visit with " + (count * -1) + " Visitors finished");
					clientForm.deleteLastVisit();
				}
			} catch (IOException e) {
			...
			try {
				...
				int availableVisitors = sin.readInt();
				
				clientForm.refreshVisitors(availableVisitors);
